...
namespace JKWatcher.RandomHelpers
{
	...
	class SaberAnimationStuff
    {
		...
		public static byte[] GenerateAnimSaberData<T>(byte[] binarySurfaceBoltSaberData, string[] animCfg) where T : Enum, IConvertible
		{
			List<surfaceBoltFrameInfo> frameInfos = new List<surfaceBoltFrameInfo>();

			using(MemoryStream ms = new MemoryStream(binarySurfaceBoltSaberData))
            {
				using (BinaryReader br = new BinaryReader(ms))
                {
					int frameInfoCount = binarySurfaceBoltSaberData.Length / Marshal.SizeOf(typeof(surfaceBoltFrameInfo));
					for(int i = 0; i < frameInfoCount; i++)
                    {
						frameInfos.Add(br.ReadBytesAsType<surfaceBoltFrameInfo>());
					}
